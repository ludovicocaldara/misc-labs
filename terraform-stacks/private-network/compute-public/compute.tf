locals {
  ...
  assign_public_ip  = try(!data.oci_core_subnets.landing_zone_subnet.subnets[0].prohibit_public_ip_on_vnic, false)
  cloud_init_user_data = base64encode(<<-CLOUDINIT
    #cloud-config
    package_update: true
    packages:
      - git
...
resource "oci_core_instance" "compute" {
  ...
  create_vnic_details {
    subnet_id                 = data.oci_core_subnets.landing_zone_subnet.subnets[0].id
