...
variable "primary_availability_domain" {
  description = "Availability Domain for the primary ExaDB-XS resources (e.g., PHX-AD-1)"
  type        = string
}

variable "standby_availability_domain" {
  description = "Availability Domain for the standby ExaDB-XS resources (e.g., PHX-AD-2)"
  type        = string
}
...
# -----------------
# Exascale Storage Vaults (one per Availability Domain)
# -----------------

resource "oci_database_exascale_db_storage_vault" "vault_primary" {
  compartment_id      = var.compartment_ocid
  availability_domain = var.primary_availability_domain
  display_name        = "exadb-xs-vault-primary"
...
resource "oci_database_exascale_db_storage_vault" "vault_standby" {
  compartment_id      = var.compartment_ocid
  availability_domain = var.standby_availability_domain
  display_name        = "exadb-xs-vault-standby"
...
# -----------------
# Two ExaDB‑XS VM Clusters across Availability Domains (in PUBLIC subnet)
# -----------------
...
resource "oci_database_exadb_vm_cluster" "cluster_a" {
  ...
  exascale_db_storage_vault_id = oci_database_exascale_db_storage_vault.vault_primary.id
...
resource "oci_database_exadb_vm_cluster" "cluster_b" {
  ...
  subnet_id                    = oci_core_subnet.subnet_public.id
  backup_subnet_id             = oci_core_subnet.subnet_backup.id
  exascale_db_storage_vault_id = oci_database_exascale_db_storage_vault.vault_standby.id
...
output "primary_storage_vault_id" {
  value = oci_database_exascale_db_storage_vault.vault_primary.id
}

output "standby_storage_vault_id" {
