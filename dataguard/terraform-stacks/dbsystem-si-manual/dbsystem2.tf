resource "oci_database_db_system" "db_system2" {
  ...
  db_home {
    database {
      ...
      #db_unique_name = "c${var.lab_name}_site2"
...
resource "null_resource" "db_system_provisioner2" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      "scp  -o StrictHostKeyChecking=no ${local.repo_script}.${oci_database_db_system.db_system2.hostname} ${oci_database_db_system.db_system2.hostname}.${oci_database_db_system.db_system2.domain}:${local.repo_script}",
      "ssh  -o StrictHostKeyChecking=no opc@${oci_database_db_system.db_system2.hostname}.${oci_database_db_system.db_system2.domain} chmod +x ${local.repo_script}",
      "ssh  -o StrictHostKeyChecking=no opc@${oci_database_db_system.db_system2.hostname}.${oci_database_db_system.db_system2.domain} sudo ${local.repo_script}",
    ]
