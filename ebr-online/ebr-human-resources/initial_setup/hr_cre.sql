...
ON regions$0 (region_id);

ALTER TABLE regions$0
...
CREATE EDITIONING VIEW countries as
    SELECT country_id, country_name, region_id FROM countries$0;

ALTER TABLE countries$0
ADD ( CONSTRAINT countr_reg_fk
        	 FOREIGN KEY (region_id)
          	  REFERENCES regions$0(region_id) 
    ) ;
...
CREATE EDITIONING VIEW locations AS
    SELECT location_id, street_address, postal_code, city, state_province, country_id FROM locations$0;

CREATE UNIQUE INDEX loc_id_pk
ON locations$0 (location_id) ;

ALTER TABLE locations$0
ADD ( CONSTRAINT loc_id_pk
       		 PRIMARY KEY (location_id)
    , CONSTRAINT loc_c_id_fk
       		 FOREIGN KEY (country_id)
        	  REFERENCES countries$0(country_id) 
    ) ;
...
CREATE EDITIONING VIEW departments AS
    SELECT department_id, department_name, manager_id, location_id FROM departments$0;

CREATE UNIQUE INDEX dept_id_pk
ON departments$0 (department_id) ;
...
ADD ( CONSTRAINT dept_id_pk
    ...
    , CONSTRAINT dept_loc_fk
        	  ...
        	  REFERENCES locations$0 (location_id)
     ) ;
...
CREATE EDITIONING VIEW jobs AS
    SELECT job_id, job_title, min_salary, max_salary FROM jobs$0;
...
CREATE EDITIONING VIEW employees AS
    SELECT employee_id, first_name, last_name, email, phone_number, hire_date, job_id, salary, commission_pct, manager_id, department_id FROM employees$0;

CREATE UNIQUE INDEX emp_emp_id_pk
ON employees$0 (employee_id) ;
...
ADD ( CONSTRAINT     emp_emp_id_pk
    ...
    , CONSTRAINT     emp_dept_fk
                     FOREIGN KEY (department_id)
                      REFERENCES departments$0
    , CONSTRAINT     emp_job_fk
                     FOREIGN KEY (job_id)
                      REFERENCES jobs$0 (job_id)
    , CONSTRAINT     emp_manager_fk
                     FOREIGN KEY (manager_id)
                      REFERENCES employees$0
    ) ;

ALTER TABLE departments$0
ADD ( CONSTRAINT dept_mgr_fk
      		 FOREIGN KEY (manager_id)
      		  REFERENCES employees$0 (employee_id)
    ) ;
...
CREATE EDITIONING VIEW job_history AS
    SELECT employee_id, start_date, end_date, job_id, department_id FROM job_history$0;

CREATE UNIQUE INDEX jhist_emp_id_st_date_pk 
ON job_history$0 (employee_id, start_date) ;

ALTER TABLE job_history$0
